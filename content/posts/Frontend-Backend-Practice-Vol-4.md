---
title: 前后端交互工程实践指南 (Vol.4：外部 API 接入)
date: 2026-04-07 01:50:00
categories: 
  - Tech
  - Tutorial
tags:
...
本文档建立在 Vol.1、Vol.2 与 Vol.3 之上，还未完成前序内容的请先完成<a href="frontend-backend-practice-vol-1.html" target="_blank">Vol.1</a>、<a href="frontend-backend-practice-vol-2.html" target="_blank">Vol.2</a>与<a href="frontend-backend-practice-vol-3.html" target="_blank">Vol.3</a> 前序教程。通过之前的实践，我们已经完成了使用 Gin 框架并结合 Nginx 代理搭建网站的基础架构。
...
我们将 Vol.1 中的<a href="frontend-backend-practice-vol-1.html#%E6%A8%A1%E5%9D%97%E4%BA%94%EF%BC%9A%E5%89%8D%E5%90%8E%E7%AB%AF%E4%BA%A4%E4%BA%92%E5%AE%9E%E4%BE%8B2" target="_blank">前后端交互实例 2</a>进行升级，把后端服务从原生逻辑改进为接入 Gemini 模型 。
...
<div style="padding: 15px; border: 1px dashed #555; background: #1e1e1e; border-radius: 8px; margin: 20px 0;">
  <h4 style="margin-top:0; color:#fff;">访问控制区</h4>
  <p style="color:#aaa;">此 Key 已被加密，请输入邀请码解锁。</p>
  <button id="unlockBtn" onclick="unlockSecret()" style="padding: 8px 15px; background: #007bff; color: white; border: none; border-radius: 4px; cursor: pointer;">点击输入密码</button>
  
  <p id="secretResult" style="display: none; color: #28a745; font-weight: bold; margin-top: 15px; font-family: monospace;"></p>
</div>

<script src="https://cdnjs.cloudflare.com/ajax/libs/crypto-js/4.1.1/crypto-js.min.js"></script>
<script>
  // 注意：此处存放的是 AES 加密后的密文，不会造成明文泄漏
  const encryptedData = "U2FsdGVkX1+vH+WyyWuUjjvRAe+HFSr6xKLUb035Wkz29dM19IIk9rEonmYX/4T9pcG0Tk0FPvuJp3Ua05zu8g=="; 
  ...
  function unlockSecret() {
    ...
    try {
      ...
      if (originalText) {
        document.getElementById('secretResult').innerText = "你的API Key: " + originalText;
...
也可访问[https://aistudio.google.com/api-keys](https://aistudio.google.com/api-keys)获取属于你的API Key。
...
### Step 5: 部署与访问
启动 Go 服务并确保 Nginx 正常运行，访问  <a href="http://47.101.141.52:888/" target="_blank">`http://47.101.141.52:888/`</a>。至此，使用 Nginx 代理部署到服务器上、属于你的第一个调用第三方 API 的网站就正式上线了！
